...
static esp_err_t httpd_accept_conn(struct httpd_data *hd, int listen_fd)
{
    ...
    if (g_current_fd > 0) {
        ESP_LOGI(TAG, "close old session to acccept new one");
        close(g_current_fd);
    ...
    if (new_fd < 0) {
        /* Err here */
        ESP_LOGW(TAG, LOG_FMT("error in accept (%d)-%s"), errno, strerror(errno));
        /* wanna restart tcp again */
        close(new_fd);
        return ESP_FAIL;
    }
    ...
    while ((fd = httpd_sess_iterate(hd, fd)) != -1) {
        if (FD_ISSET(fd, &read_set) || (httpd_sess_pending(hd, fd))) {
            ...
            if (httpd_sess_process(hd, fd) != ESP_OK) {
                ESP_LOGI(TAG, LOG_FMT("process incomming HTTP request fail. socket %d"), fd);
                if (fd > 0)
                    close(fd);
